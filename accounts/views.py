...
def update_condition_set(request, set_pk=None): # set_pk があれば編集、なければ追加
    ...
    if set_pk:
        ...
        existing_cond_ids = list(Condition_set_items.objects.filter(
    ...
    if request.method == "GET":                     
        # 今の選択状況をsessionに保存
        request.session["old_selected_cond"] = existing_cond_ids
    ...
    # どれかのボタンを押されたとき
        ...
        if action == "cond_active":
            ...
            old_selected_cond_ids = request.session.get("old_selected_cond") or [] 
            ...
            if new_selected_cond_id:
                ...
                for cid in active_condition_ids:
                    ...
                    if c.condition_category_id != new_selected_cond.condition_category_id:
                        ...
                        print(c.condition_category_id, new_selected_cond.condition_category_id, "追加")
                # 新しく選択された状況を追加
                filtered_selected.append(new_selected_cond_id[0])  
                existing_cond_ids = filtered_selected
        ...
        # 状況ボタンのunactiveになった場合
        elif action == "cond_unactive":
            selected_c = data.get("selected_cond") 
            active_condition_ids = [int(x) for x in selected_c if str(x).isdigit()] 
            old_selected_cond_ids = active_condition_ids
            existing_cond_ids = active_condition_ids
            print(existing_cond_ids)
        ...
        # 保存ボタンを押されたとき
        elif action == "save_condition_set":
            # 状況名と選択した条件を取得
            raw = QueryDict('', mutable=True)
            raw.update({
                "name": data.get("name"),
            })
            condition_set_form = ConditionSetForm(data=raw, instance=set_data)
